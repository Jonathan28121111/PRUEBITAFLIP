...
public class GameWindow extends JFrame implements GameClient.GameClientListener {
    ...
    private void updatePlayerPanels(GameState s) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            if (playerPanels.size() != players.size()) {
    ...
    public void onLoginSuccess(User user) {
        SwingUtilities.invokeLater(() -> {
            ...
            showPanel("lobby");
            infoPanel.log("Bienvenido " + user.getUsername());
    ...
    public void onRegisterSuccess(User user) {
        SwingUtilities.invokeLater(() -> {
            // Resetear botones primero
            loginPanel.resetButtons();
            
            // Mostrar mensaje de éxito
            JOptionPane.showMessageDialog(this, 
                "Usuario '" + user.getUsername() + "' registrado correctamente!", 
                "Registro Exitoso", 
    ...
    public void onPlayerLeft(int id, String name) { 
        SwingUtilities.invokeLater(() -> {
            infoPanel.log("- " + name + " salio");
            
            // Quitar el panel del jugador que salió
    ...
    public void onGameEnd(java.util.List<Player> players, int winnerId) {
        SwingUtilities.invokeLater(() -> {
            gameStarted = false; isMyTurn = false; updateControls();
            Player w = null; for (Player p : players) if (p.getId() == winnerId) { w = p; break; }
            String msg = w != null ? w.getName() + " GANA con " + w.getTotalScore() + " pts!" : "Fin";
            infoPanel.log("\n*** " + msg + " ***\n");
            
            // Mostrar diálogo de revancha
            if (!isSpectator) {
                ...
                if (option == 0) {
                    // Revancha - volver a sala de espera
                ...
                if (option == 0) {
                    // Seguir observando - ir a sala de espera
                    waitingRoomPanel.reset();
                    showPanel("waiting");
                } else {
                    // Salir
